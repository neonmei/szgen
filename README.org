...
*szgen* is a basic CLI tool for generating synthetic OpenTelemetry data inspired by tools like ~telemetrygen~ or ~dogshell~. Why I made this? I wanted to quickly throw metrics at a collector to test/debug a custom OTel processor, eventually I also started using it for measuring stuff and generating metrics in bash scripts.
